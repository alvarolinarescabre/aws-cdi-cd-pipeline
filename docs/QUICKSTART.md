...
- Buildspec location: services/pipeline/buildspec.yml
...
| Buildspec location | services/pipeline/buildspec.yml | Monorepo structure |
...
| buildspec.yml not found | Especifica: `services/pipeline/buildspec.yml` en CodeBuild |

---

## Archivos del Proyecto (Monorepo)

### Apps
- **apps/api/app.py**: Aplicación Flask
- **apps/api/Dockerfile**: Empaqueta tu app
- **apps/api/requirements.txt**: Dependencias Python

### Services
- **services/pipeline/buildspec.yml**: Motor que CodeBuild ejecuta

### Infrastructure
- **infrastructure/.env.example**: Variables de configuración
- **infrastructure/scripts/test-build.sh**: Test local Docker
- **infrastructure/scripts/pre-commit-check.sh**: Validación pre-push

---

**¿Necesitas ayuda?** Revisa `docs/README.md` para troubleshooting detallado.
